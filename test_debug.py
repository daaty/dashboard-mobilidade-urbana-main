...
async def test_connection():
    try:
        ...
        print('\n=== VERIFICAÇÃO TAXA ACEITAÇÃO ===')
        result = await conn.fetchrow('''
            SELECT 
                AVG(CAST(additional_data->>'acceptance_rate' AS DECIMAL)) as taxa_media,
                COUNT(*) as motoristas_com_taxa
            FROM drivers_data 
            WHERE additional_data->>'acceptance_rate' IS NOT NULL 
            AND additional_data->>'acceptance_rate' != ''
            AND additional_data->>'acceptance_rate' != '0'
        ''')
        print(f'Taxa média: {result[0] or 0}%')
        print(f'Motoristas com taxa: {result[1]}')
        ...
        result = await conn.fetch('''
            SELECT 
                ...
                additional_data->>'acceptance_rate' as acceptance
            FROM drivers_data 
        ...
        for row in result:
            print(f"Driver {row[0]}: Receita={row[1]}, Corridas={row[2]}, Distância={row[3]}, Taxa={row[4]}")
