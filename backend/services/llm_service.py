...
class LLMService:
    ...
    def __init__(self):
        ...
        if self.api_key:
            ...
            print("GEMINI_API_KEY não configurada, usando respostas mock")
            self.enabled = False
    ...
    def _mock_chat_response(self, question: str) -> Dict[str, Any]:
        ...
        # Perguntas específicas sobre dados
            ...
            try:
                ...
                try:
                    response_obj = get_overview()
                    dashboard_data = response_obj.get_json().get('data', {})
                    ...
                    if total_corridas > 0:
                        ...
                        response = f"🚗 **Análise de Corridas (dados reais):**\n\n"
                        response += f"• **Total:** {total_corridas:,} corridas\n"
                        response += f"• **Concluídas:** {corridas_concluidas:,} ({taxa_sucesso:.1f}%)\n"
                        
                        if taxa_sucesso < 80:
                            response += "\n⚠️ **Alerta:** Taxa de sucesso baixa. Recomendo investigar causas dos cancelamentos."
                        else:
                            response += "\n✅ **Boa performance:** Taxa de sucesso adequada!"
                    else:
                        response = "📊 **Status:** Nenhuma corrida registrada ainda. Para começar a analisar:\n1. Importe dados históricos\n2. Registre corridas ativas\n3. Configure motoristas no sistema"
                
                except Exception:
                    response = "🚗 Sobre as corridas: analisando dados em tempo real... Sistema carregando métricas."
            ...
            # ANÁLISE REAL DOS DADOS FINANCEIROS
            try:
                from backend.api.dashboard import get_overview
                ...
                try:
                    ...
                    if receita_total > 0:
                        ...
                        response = f"💰 **Análise Financeira (dados reais):**\n\n"
                        response += f"• **Receita Total:** R$ {receita_total:,.2f}\n"
                        response += f"• **Receita/Corrida:** R$ {receita_media:.2f}\n"
                        response += f"• **Corridas Pagas:** {corridas_concluidas:,}\n\n"
                        
                        if receita_media < 15:
                            response += "📈 **Oportunidade:** Ticket médio baixo. Considere reajustar tarifas."
                        else:
                            response += "✅ **Bom desempenho:** Ticket médio adequado para o mercado."
                    else:
                        response = "💰 **Receita:** R$ 0,00 - Sistema ainda sem movimentação financeira. Prioridades:\n1. Ativar corridas pagas\n2. Configurar tarifas\n3. Integrar pagamentos"
                
                except Exception:
                    response = "💰 Análise financeira: carregando dados em tempo real..."
            ...
            try:
                ...
                try:
                    ...
                    if motoristas_ativos > 0:
                        corridas_por_motorista = total_corridas / motoristas_ativos if motoristas_ativos > 0 else 0
                        response = f"👥 **Gestão de Motoristas (dados reais):**\n\n"
                        response += f"• **Ativos:** {motoristas_ativos} motoristas\n"
                        response += f"• **Produtividade:** {corridas_por_motorista:.1f} corridas/motorista\n"
                        
                        if avaliacao_media > 0:
                            response += f"• **Avaliação Média:** {avaliacao_media:.1f}/5⭐\n\n"
                        
                        if corridas_por_motorista < 10:
                            response += "📊 **Insight:** Baixa produtividade. Implemente incentivos ou treinamentos."
                        else:
                            response += "✅ **Equilibrado:** Produtividade dentro do esperado."
                    else:
                        response = "👥 **Motoristas:** Nenhum ativo no momento. Plano de ação:\n1. Recrutamento ativo\n2. Programa de incentivos\n3. Treinamento de uso do app\n**Meta:** 10-20 motoristas para início"
                
                except Exception:
                    response = "👥 Gestão de motoristas: analisando dados da equipe..."
